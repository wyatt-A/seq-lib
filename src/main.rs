...
fn main() {
    ...
    let p = Dwi3DParams::default();
    let mut state = p.adjustment_state();
    *state.get_mut("diff_x").unwrap() = FieldGrad::tesla_per_meter(2.3).si();
    *state.get_mut("diff_y").unwrap() = FieldGrad::tesla_per_meter(2.3).si();
    *state.get_mut("diff_z").unwrap() = FieldGrad::tesla_per_meter(2.3).si();
